...
extension Reactive where Base: SessionManager {
	...
	public func request(_ method: Alamofire.HTTPMethod, _ url: URLConvertible, parameters: [String: Any]? = nil, encoding: ParameterEncoding = URLEncoding.default, headers: [String: String]? = nil) -> Observable<HTTPURLResponse> {
...
extension Reactive where Base: DataRequest {
	...
	public func raw(queue: DispatchQueue? = nil) -> Observable<HTTPURLResponse> {
		return Observable.create { observer in
			let request = self.base.response(queue: queue) { (packedResponse) -> Void in
